...
$(document).ready(function() {
   ...
   function delete_link_action() {  
    ...
    //set id in form  
    $('#delete_dialog input[name="id3"]').val(id);  
...
$(document).ready(function() {
    ...
    buttons: { 
      "Update": function() { 
		...
		$.post('processUpdate.php',{
			user_id: eid, user_status: estatus, user_role: erole, action:'joined'
		});//End Post
		alert("hi");
		$(this).dialog("close");
		document.location.reload();
   ...
   function edit_link_action() {  
	...
	$('#edit_dialog input[name="status1"]').val($('.status',user).html());  
    
	//set current age in form
	$('#edit_dialog input[name="role1"]').val($('.role',user).html());
...
$("#d1").dialog({
    ...
    buttons: { 
      "Add": function() { 
		...
		alert("hi");
		$.post('processAdd.php',{
			user_fname: fname,user_uname: uname,user_email: email, user_pass: pass,	user_repass:repass,	user_mob: mob,	user_gen: gen,	user_dob: dob,
			user_status: status,
			user_role:role,
		    action:'joined'
		});//End Post
		...
		$("#dob").val('');
		$(this).dialog("close");
		window.location.replace("http://localhost/reminder/admin/");		
		window.location.reload();
		},
      "Cancel": function() { 
	  	$("#id").val('');
