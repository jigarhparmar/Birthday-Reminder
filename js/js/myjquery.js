...
$(document).ready(function() {
    ...
    autoOpen:false,   
    title:"Edit Reminder Information",   
    modal:true,
	show:'explode',
	hide:'puff',
    buttons: { 
      "Update": function() { 
		...
		$.post('processUpdate.php',{
			rem_id: id, rem_title:title, rem_name:name, rem_date:date, rem_time:time, rem_desc:desc, action:'joined'
		});//End Post
		$(this).dialog("close");
		window.location.replace("http://localhost/reminder/viewremind.php");		
		window.location.reload();
		},
      "Cancel": function() { 
	  	$("#id").val('');
   ...
   function edit_link_action() {  
	...
	$('#edit_dialog input[name="title1"]').val($('.title',user).html());  
    
	//set current age in form
	$('#edit_dialog input[name="name1"]').val($('.name',user).html());
     
//set current age in form
	$('#edit_dialog input[name="date1"]').val($('.date',user).html());
   	 
	 //set current age in form
	$('#edit_dialog input[name="time1"]').val($('.time',user).html());
	
	//set current age in form
	$('#edit_dialog input[name="desc1"]').val($('.desc',user).html());
...
$("#d1").dialog({
    ...
    buttons: { 
      "Add": function() { 
		...
		$("#age").val('');				
		$(this).dialog("close");
		window.location.replace("http://localhost/v/");		
		window.location.reload();
		},
      "Cancel": function() { 
	  	$("#id").val('');
